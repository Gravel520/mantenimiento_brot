...
def grabar_registros(self, gg_records, tablas_records):
    data = []
    data.append("servicio") # Primero disponemos los datos de la tabla 'servicio'
    ...
    data.append("grabacion_general") # Después los datos de la tabla 'grabacion_general'
...
def borrar_datos(self, records):
    ...
    msg.setWindowTitle('Copia Selectiva')
        
    try:
        cur = conn.cursor()
        cur.execute(sql)
        conn.commit()
        texto = 'Id de (servicio)... borrados'    
        msg.setText(texto)
        msg.show()
    ...
    try:
        ...
        cur.execute(sql)
        conn.commit()
        msg.setText(f'        {texto}\nDatos de Grabacion_general... borrados')
        msg.show()
